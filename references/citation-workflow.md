...
| Dimension | Question answered | Failure mode it catches |
|-----------|-------------------|------------------------|
| **Entry truthfulness** | Is each `.bib` entry real, and do its fields match the actual publication? | Fabricated papers, wrong year / author / DOI, swapped to a same-titled but different paper |
| **Claim support** | Does each in-text citation actually support the claim attached to it? | Cite piles, mis-attributed numbers, topic-level false positives |
...
- [ ] Every entry has a DOI or arXiv ID that resolves.
- [ ] Every entry has at least one author surname matching the paper's actual first author.
- [ ] No two entries share an identical title but different authors (signals a mis-paste).
- [ ] Every in-text citation has a claim attached; no `\cite{}` is decorative.
